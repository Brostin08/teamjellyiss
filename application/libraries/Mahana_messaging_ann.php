...
class Mahana_messaging_ann
{
    ...
    function get_message($msg_id, $user_id)
    {
        ...
        if ($message = $this->ci->ann_model->get_message($msg_id, $user_id))
        {
    ...
    function get_full_thread($thread_id, $user_id, $full_thread = FALSE, $order_by = 'ASC')
    {
        ...
        if ($message = $this->ci->ann_model->get_full_thread($thread_id, $user_id, $full_thread, $order_by))
        {
            return $this->_success($message);
        }
    ...
    function get_all_threads($user_id, $full_thread = FALSE, $order_by = 'ASC')
    {
		...
		$message = $this->ci->ann_model->get_all_threads($user_id, $full_thread, $order_by);
        if (is_array($message))
        {
            return $this->_success($message);
        }
    ...
    function update_message_status($msg_id, $user_id, $status_id )
    {
        ...
        if ($this->ci->ann_model->update_message_status($msg_id, $user_id, $status_id))
        {
    ...
    function add_participant($thread_id, $user_id)
    {
        ...
        if ( ! $this->ci->ann_model->valid_new_participant($thread_id, $user_id))
        {
            $this->_particpant_error(MSG_ERR_PARTICIPANT_EXISTS);
        }

        if ( ! $this->ci->ann_model->application_user($user_id))
        {
            $this->_particpant_error(MSG_ERR_PARTICIPANT_NONSYSTEM);
        }

        if ($this->ci->ann_model->add_participant($thread_id, $user_id ))
        {
    ...
    function remove_participant($thread_id, $user_id)
    {
        ...
        if ($this->ci->ann_model->remove_participant($thread_id, $user_id))
        {
     ...
     * @return  array
     */
    function send_new_message($sender_id, $recipients, $subject = '', $body = '', $priority = PRIORITY_NORMAL, $g_id = NULL)
    {
        $group_id = $g_id;
        if (empty($sender_id))
        {
        ...
        if ($thread_id = $this->ci->ann_model->send_new_message($sender_id, $recipients, $subject, $body, $priority, $group_id))
        {
    ...
    function reply_to_message($msg_id, $sender_id, $subject = '', $body = '', $priority = PRIORITY_NORMAL)
    {
        ...
        if ($new_msg_id = $this->ci->ann_model->reply_to_message($msg_id, $sender_id, $body, $priority))
        {
    ...
    function get_participant_list($thread_id, $sender_id = 0)
    {
        ...
        if ($participants = $this->ci->ann_model-> get_participant_list($thread_id, $sender_id))
        {
    ...
    function get_msg_count($user_id, $status_id = MSG_STATUS_UNREAD)
    {
        ...
        if (is_numeric($message = $this->ci->ann_model->get_msg_count($user_id, $status_id)))
        {
            return $this->_success($message);
        }
